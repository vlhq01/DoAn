...
class MyFCM : FirebaseMessagingService() {
        ...
        message.data.let {
            ...
            when (type.toInt()) {
                ...
                BalanceChanges.TYPE_TRANSFERFROM -> {
                    ...
                    val date = it.getValue("date").toString()
                    Log.d(TAG, "onMessageReceived: " + date)
                    val sendername = it.getValue("sendername").toString()
                    Log.d(TAG, "onMessageReceivedsender: " + sendername)
                    val odersn = it.getValue("odersn").toString()
                    val referenceid = it.getValue("referenceid").toString()
                    showTransferFromNotification(
                        TransferFrom(
                            amount,
                            date,
                            odersn,
                    ...
                    sendransferfromtomain(
                        broadcaster,
                        TransferFrom(amount, date, odersn, sendername, referenceid)
                    )
                }
            }
        }
    ...
    private fun sendransferfromtomain(broadcastManager: LocalBroadcastManager, transferFrom: TransferFrom) {
        val intent = Intent(BALANCECHANGES)
        intent.putExtra("transferfrom", transferFrom)
        intent.putExtra("type", transferFrom.type)
        broadcastManager.sendBroadcast(intent)
    }
    ...
    private fun showTopUpNotification(data: TopUp) {
        ...
        val builder =
            NotificationCompat.Builder(this, getString(R.string.doan_notification_channel_id))
                .setSmallIcon(R.drawable.shopeepayicon).setContentTitle("Money TopUp")
    ...
    private fun showTransferNotification(data: Transfer) {
        val builder =
            NotificationCompat.Builder(this, getString(R.string.doan_notification_channel_id))
                .setSmallIcon(R.drawable.shopeepayicon).setContentTitle("Money Transfer")
    ...
    private fun showTransferFromNotification(data: TransferFrom) {


        val builder =
            NotificationCompat.Builder(this, getString(R.string.doan_notification_channel_id))
                .setSmallIcon(R.drawable.shopeepayicon).setContentTitle("Money Receive")
                .setContentText("Receive " + data.amount + " from " + data.sendername)
                .setPriority(NotificationCompat.PRIORITY_DEFAULT)
        // Set the intent that will fire when the user taps the notification
        val n = builder.build()
        val notificationManager =
    ...
    private fun showPaymentNotification(data: Payment) {
        ...
        val builder =
            NotificationCompat.Builder(this, getString(R.string.doan_notification_channel_id))
                .setSmallIcon(R.drawable.shopeepayicon).setContentTitle("Payment")
    ...
    private fun showWithDrawNotification(data: WithDraw) {
        ...
        val builder =
            NotificationCompat.Builder(this, getString(R.string.doan_notification_channel_id))
                .setSmallIcon(R.drawable.shopeepayicon).setContentTitle("With Draw")
